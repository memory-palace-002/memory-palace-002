...
/**
 * DOM 交互层: 关闭按钮、翻页器、重命名面板、物品详情、新建记忆入口。
...
export function Overlay() {
  ...
  return (
    <div className="overlay">
      {shown && (
        <>
          <button className="close-btn" aria-label="关闭" onClick={() => closeBook()}>
            ×
          </button>
          {openBox && pages > 1 && (
            <div className="pager">
          ...
          <div className="hint">Esc 或点击空白处关闭 · 点击书页翻页 · 拖动物品可取出</div>
        </>
      )}

      {/* 新建记忆: 外壳按钮, 数据走 ingestCapture 接口(队友模块接入后自动生效) */}
      <button className="capture-btn" onClick={() => capture(demoCapture())}>
        ＋ 新建记忆
      </button>
      ...
      {detailItem && (
        <div className="modal-backdrop" onClick={() => closeDetail()}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <button className="close-btn modal-close" aria-label="关闭" onClick={() => closeDetail()}>
              ×
            </button>
            ...
            {detailItem.kind === 'note' && <p className="note-body">{detailItem.text}</p>}
            ...
            {detailItem.extracted && (
              <div className="actions">
                <button className="primary" onClick={() => returnItem(detail!.boxId, detailItem.id)}>
                  放回书里
                </button>
              </div>
            )}
...
function RenamePanel({ initial }: { initial: string }) {
  ...
  return (
    <div className="rename-panel">
      ...
      />
      <button className="primary" onClick={() => renameCommit(value)}>
        确定
      </button>
