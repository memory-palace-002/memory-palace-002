...
export function ExtractedLayer() {
  ...
  useFrame((state) => {
    ...
    floorPoint.x = THREE.MathUtils.clamp(floorPoint.x, -3, 3);
    floorPoint.z = THREE.MathUtils.clamp(floorPoint.z, -2.05, 1.7);
...
function RoomItem({ boxId, item }: { boxId: string; item: CollectionItem }) {
  ...
  return (
    <group ref={ref} position={target} onPointerDown={onPointerDown} onClick={onClick}>
      <group position={[0, ROOM_ITEM_SIZE / 2, 0]}>
