/**
 * 记忆书柜状态机（移植自队友的 memory-bookshelf 项目，zustand store 原样保留）。
 *
 * 与原项目的差异（适配本仓库 3D 回忆房间）：
 * - 相机不再由本模块接管：本仓库 RoomBackground 的 CinematicRig 是固定室内机位，
 *   打开书时书自己飞到「室内机位正前方」的展示位（STAGE_BOOK），
 *   相机保持不动（MemoryRoom3D 会把 cameraPaused 置真冻结转头）。
 * - 书的初始机位字段保留但仅作文档用途。
 */
...
/** 初始机位：本仓库室内机位（RoomBackground 的 IN，只作文档用途，不再接管相机） */
export const INITIAL_POSE: CameraPose = {
  position: [0, 1.5, 2.45],
  target: [-0.42, 1.0, -2.45],
}

/** 阅读机位：相机保持室内机位，视线看向展示位的书（只作文档用途） */
export const STAGE: CameraPose = {
  position: [0, 1.5, 2.45],
  target: [-0.1, 1.38, 1.3],
}

/** 打开时书本飞到的展示位姿（室内机位正前方 ~1.16m，封面朝镜头） */
export const STAGE_BOOK = {
  position: [-0.1, 1.38, 1.3] as [number, number, number],
